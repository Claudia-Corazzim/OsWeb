...
def ordens_servico():
    ...
    if request.method == 'POST':
        ...
        conn.execute('INSERT INTO ordens_servico (cliente_id, descricao, data) VALUES (?, ?, ?)',
                     (cliente_id, descricao, data))
        conn.commit()
        return redirect(url_for('ordens_servico'))
    os_list = conn.execute('''
        SELECT os.id, c.nome AS cliente, os.descricao, os.data, os.cliente_id
...
def editar_os(id):
    ...
    if request.method == 'POST':
        ...
        descricao = request.form['descricao']
        data = request.form['data']
        conn.execute('UPDATE ordens_servico SET cliente_id = ?, descricao = ?, data = ? WHERE id = ?',
                     (cliente_id, descricao, data, id))
        conn.commit()
        conn.close()
        return redirect(url_for('ordens_servico'))
